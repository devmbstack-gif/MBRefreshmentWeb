...
import { CalendarDays, CheckCircle2, ClipboardList, Pencil, Plus, Sparkles, Trash2, Users } from 'lucide-react';
        ...
        <div className="flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg">
            ...
            <div className="mt-auto border-t border-slate-200 pt-4">
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-4">
                    <Button size="sm" variant="outline" onClick={onEdit} className="rounded-xl">
...
function DeletePlanModal({ plan, onClose }: { plan: Plan; onClose: () => void }) {
    return (
        <Dialog open={true} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        "{plan.title}" will be deleted permanently. You can only delete inactive plans without usage history.
                    </DialogDescription>
                </DialogHeader>
...
function StatusPlanModal({ plan, onClose }: { plan: Plan; onClose: () => void }) {
        ...
        <Dialog open={true} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Are you sure you want to {action.toLowerCase()} "{plan.title}"?
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter className="gap-2">
                    <DialogClose asChild>
...
function CreatePlanModal({
    ...
    function updateQuantity(itemId: number, qty: number) {
        const itemStock = availableItems.find((item) => item.id === itemId)?.stock_quantity ?? 1;
        const safeQty = Math.max(1, Math.min(itemStock, qty || 1));
        setSelectedItems((prev) =>
            prev.map((i) => (i.item_id === itemId ? { ...i, quantity: safeQty } : i))
        );
    }
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        for (const selected of selectedItems) {
            ...
            if (selected.quantity > item.stock_quantity) {
                form.setError('items', `${item.name} quantity cannot be greater than stock (${item.stock_quantity}).`);
                return;
            }
        }
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <Input
                                                    type="number"
                                                    min={1}
                                                    max={availableItems.find((i) => i.id === item.item_id)?.stock_quantity ?? 1}
                                                    value={item.quantity}
                                                    onChange={(e) => updateQuantity(item.item_id, parseInt(e.target.value) || 1)}
                                                    className="h-7 w-16 text-center"
                                                />
                                                <span className="text-xs text-slate-500">
                                                    / {availableItems.find((i) => i.id === item.item_id)?.stock_quantity ?? 0}
                                                </span>
                                            </div>
                                            <button
                                                type="button"
...
function EditPlanModal({
    ...
    const [selectedItems, setSelectedItems] = useState<{ item_id: number; item_name: string; quantity: number }[]>(
        plan.items.map((item) => ({ item_id: item.item_id, item_name: item.item_name, quantity: item.quantity })),
    );
    const [selectedItemId, setSelectedItemId] = useState('');
    ...
    function addItem() {
        ...
        setSelectedItems((prev) => [...prev, { item_id: id, item_name: item.name, quantity: 1 }]);
        setSelectedItemId('');
    }

    function updateQuantity(itemId: number, qty: number) {
        const stock = availableItems.find((i) => i.id === itemId)?.stock_quantity ?? 1;
        const safeQty = Math.max(1, Math.min(stock, qty || 1));
        setSelectedItems((prev) => prev.map((item) => (item.item_id === itemId ? { ...item, quantity: safeQty } : item)));
    }
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        for (const selected of selectedItems) {
            ...
            if (!item || selected.quantity > item.stock_quantity) {
                form.setError('items', `Invalid quantity for ${selected.item_name}.`);
                return;
            }
        }
        form.transform((data) => ({
    ...
    return (
        <Dialog open={true} onOpenChange={onClose}>
            <DialogContent className="max-h-[92vh] overflow-y-auto rounded-3xl border border-emerald-100 bg-white p-0 shadow-2xl [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:max-w-2xl">
                <DialogHeader className="border-b border-emerald-100 bg-gradient-to-r from-white to-emerald-50 px-6 py-5">
                    <DialogTitle className="text-2xl font-semibold text-slate-900">Edit Month Plan</DialogTitle>
                    <DialogDescription className="text-sm text-slate-500">
                ...
                <form onSubmit={handleSubmit} className="space-y-5 p-6">
                    <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-5 space-y-4">
                        ...
                        <div className="grid gap-4 sm:grid-cols-3">
                            <div className="space-y-1.5">
                                <Label>Period Type</Label>
                                <Select value={form.data.period_type} onValueChange={(val) => form.setData('period_type', val)}>
                                    <SelectTrigger>
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                            ...
                            <div className="space-y-1.5">
                                <Label>End Date</Label>
                                <Input type="date" value={form.data.ends_at} onChange={(e) => form.setData('ends_at', e.target.value)} />
                            </div>
                        </div>
                    </div>
                    <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-5 space-y-3">
                        ...
                        <div className="flex gap-2">
                            <Select value={selectedItemId} onValueChange={setSelectedItemId}>
                                ...
                                <SelectContent>
                                    {availableItems
                                        .filter((item) => item.stock_quantity > 0)
                                        .filter((item) => !selectedItems.find((selected) => selected.item_id === item.id))
                                        .map((item) => (
                                            <SelectItem key={item.id} value={String(item.id)}>
                                                {item.name} (Stock: {item.stock_quantity})
                                            </SelectItem>
                                ...
                                {selectedItems.map((item) => (
                                    <div key={item.item_id} className="flex items-center justify-between px-4 py-3">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <button type="button" onClick={() => removeItem(item.item_id)} className="text-muted-foreground hover:text-red-500">
                                                <Trash2 className="h-4 w-4" />
                                            </button>
                                        </div>
                                    </div>
...
function AssignPlanModal({
    ...
    const form = useForm({
        ...
        allocations: [] as { employee_id: number; item_id: number; quantity: number }[],
    });
    ...
    function setAllocationValue(employeeId: number, itemId: number, quantity: number) {
        const safe = Math.max(0, Math.floor(quantity || 0));
        setAllocationMatrix((prev) => ({ ...prev, [key(employeeId, itemId)]: safe }));
    }
    ...
    function distributeEqually() {
        ...
        for (const item of plan.items) {
            ...
            selected.forEach((employeeId, index) => {
                next[key(employeeId, item.item_id)] = base + (index < remainder ? 1 : 0);
            });
        }
    ...
    function currentAssignedForItem(itemId: number) {
        return form.data.employee_ids.reduce((sum, employeeId) => sum + (allocationMatrix[key(employeeId, itemId)] ?? 0), 0);
    }
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        for (const item of plan.items) {
            ...
            if (assigned > item.quantity) {
                form.setError('allocations', `${item.item_name} assigned quantity cannot exceed ${item.quantity}.`);
                return;
            }
        }
    ...
    return (
        <Dialog open={true} onOpenChange={onClose}>
            <DialogContent className="max-h-[92vh] overflow-y-auto rounded-3xl border border-emerald-100 bg-white p-0 shadow-2xl [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:max-w-5xl">
                <DialogHeader className="border-b border-emerald-100 bg-gradient-to-r from-emerald-500 via-emerald-500 to-teal-500 px-6 py-5 text-white">
                    <DialogTitle className="text-2xl font-semibold text-white">Assign Month Plan</DialogTitle>
                    <DialogDescription className="text-sm text-emerald-50">
                        {plan.title} · Select employees and distribute each item quantity exactly.
                    </DialogDescription>
                </DialogHeader>

                <form onSubmit={handleSubmit} className="space-y-5 p-6">
                    {form.errors.employee_ids && (
                        <p className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-medium text-rose-700">{form.errors.employee_ids}</p>
                    )}
                    {form.errors.allocations && (
                        <p className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-medium text-rose-700">{form.errors.allocations}</p>
                    )}
                    ...
                    <div className="grid gap-4 lg:grid-cols-[1.15fr_1.85fr]">
                        <section className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/50 p-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                <Button type="button" size="sm" variant="outline" onClick={selectAll} className="rounded-xl">
                                    Select All
                                </Button>
                                <Button type="button" size="sm" variant="ghost" onClick={clearAll} className="rounded-xl">
                                    Clear
                                </Button>
                                <Button type="button" size="sm" variant="ghost" onClick={distributeEqually} className="rounded-xl">
                                    Distribute Equally
                                </Button>
                            </div>
                                            ...
                                            return (
                                                <>
                                                    ...
                                                    <Avatar className="h-9 w-9 border border-emerald-200">
                                                        {employee.avatar ? <AvatarImage src={employee.avatar} alt={employee.name} className="object-cover" /> : null}
                        ...
                        <section className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/50 p-4">
                            <div className="flex items-center justify-between">
                                <h3 className="text-sm font-semibold text-slate-900">Item Allocation</h3>
                                <p className="text-xs font-medium text-slate-500">Set quantity per employee</p>
                            </div>
                            {form.data.employee_ids.length === 0 ? (
                                <div className="rounded-xl border border-dashed border-slate-300 bg-white px-4 py-8 text-center text-sm text-slate-500">
                                    Select employees to start assigning item quantities.
                                </div>
                            ) : (
                                <div className="max-h-[460px] space-y-3 overflow-y-auto pr-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                                    {plan.items.map((item) => {
                                        const assigned = currentAssignedForItem(item.item_id);
                                        return (
                                            <div key={item.item_id} className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                                                <div className="flex items-center justify-between">
                                                    <p className="text-sm font-semibold text-slate-800">{item.item_name}</p>
                                                    <p
                                                        className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
                                                            assigned > item.quantity ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'
                                                        }`}
                                                    >
                                                ...
                                                <div className="grid gap-2.5 md:grid-cols-2">
                                                    {form.data.employee_ids.map((employeeId) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div className="flex min-w-0 items-center gap-2 pr-2">
                                                                    ...
                                                                    <span className="truncate text-xs font-medium text-slate-700">{employee.name}</span>
                                                                </div>
                                                                <Input
                                                                    type="number"
                                                                    min={0}
                                                                    value={allocationMatrix[key(employeeId, item.item_id)] ?? 0}
                                                                    onChange={(e) => setAllocationValue(employeeId, item.item_id, Number(e.target.value) || 0)}
                    ...
                    <div className="flex justify-end gap-3 border-t border-slate-200 pt-4">
                        ...
                        <Button
                            type="submit"
                            disabled={form.processing || form.data.employee_ids.length === 0}
