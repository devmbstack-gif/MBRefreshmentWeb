...
import { Briefcase, CalendarDays, Mail, Pencil, Plus, Power, Trash2, Upload, UserRound } from 'lucide-react';
...
const defaultDepartments = ['Engineering', 'Design', 'HR', 'Management', 'Marketing', 'Sales', 'Finance', 'Other'];

const avatarColors = ['bg-emerald-500', 'bg-green-600', 'bg-teal-600', 'bg-lime-600', 'bg-emerald-600', 'bg-green-500'];
...
export default function AdminEmployees({ employees }: Props) {
    ...
    const departmentOptions = Array.from(
        new Set([...defaultDepartments, ...employees.map((employee) => employee.department).filter(Boolean) as string[]]),
    ...
    return (
        <>
                    ...
                    <div className="flex flex-col gap-6 px-6 py-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
                        <div className="max-w-2xl">
                            ...
                            <p className="mt-2 text-sm leading-6 text-slate-600">
                                Manage employee accounts with a cleaner card layout, quick actions, and a softer visual style.
                            </p>
                        </div>
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                            <SummaryCard label="Total" value={employees.length} />
                            <SummaryCard label="Active" value={activeEmployees} />
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            {activeEmployees} active employees and {inactiveEmployees} inactive employees.
                        </p>
                    </div>
                ...
                <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                    {employees.length === 0 ? (
                        <div className="col-span-full rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center shadow-sm">
                            ...
                            <p className="mt-2 text-sm text-slate-500">Create your first employee to start managing the team.</p>
                        ...
                        employees.map((employee) => (
                            <div key={employee.id} className="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg">
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div className="-mt-8 flex items-center gap-3">
                                            {employee.avatar ? (
                                                ...
                                                <div className={`flex h-16 w-16 items-center justify-center rounded-2xl border-4 border-white ${getAvatarGradient(employee.name)} text-lg font-bold text-white shadow-md`}>
            ...
            {statusEmployee && (
                <StatusConfirmModal employee={statusEmployee} onClose={() => setStatusEmployee(null)} />
            )}

            {deleteEmployee && (
                <DeleteEmployeeModal employee={deleteEmployee} onClose={() => setDeleteEmployee(null)} />
            )}
        </>
    );
}
...
function StatusConfirmModal({ employee, onClose }: { employee: Employee; onClose: () => void }) {
        ...
        <Dialog open={true} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Are you sure you want to {action.toLowerCase()} {employee.name}? You can change this again later.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter className="gap-2">
                    ...
                    <Button
                        ...
                        onClick={() => {
                            router.patch(`/admin/employees/${employee.id}/toggle-status`);
                            onClose();
                        }}
                    >
                        {action}
...
function DeleteEmployeeModal({ employee, onClose }: { employee: Employee; onClose: () => void }) {
    return (
        <Dialog open={true} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        This will permanently delete {employee.name}, the employee account, image, quota data, and usage history. This action cannot be undone.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter className="gap-2">
                    <DialogClose asChild>
...
function EmployeeInfoRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
    return (
            ...
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                    {previewSrc ? (
                        ...
                        <div className={`flex h-20 w-20 items-center justify-center rounded-2xl border-4 border-white ${getAvatarGradient(name || 'Employee')} text-xl font-bold text-white shadow-sm`}>
        ...
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="max-h-[92vh] overflow-y-auto rounded-3xl border border-emerald-100 bg-white p-0 shadow-2xl [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:max-w-2xl">
                <DialogHeader className="border-b border-emerald-100 bg-gradient-to-r from-white to-emerald-50 px-6 py-5">
                    <DialogTitle className="text-2xl font-semibold text-slate-900">Add Employee</DialogTitle>
                    <DialogDescription className="text-sm text-slate-500">
                        Create a new employee account with profile details, login credentials, and an optional image.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 p-6">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-5">
                        ...
                        <FormField label="Joining Date" error={form.errors.joining_date}>
                            <Input
                                type="date"
...
function EditEmployeeModal({
    ...
    return (
        <Dialog open={true} onOpenChange={onClose}>
            <DialogContent className="max-h-[92vh] overflow-y-auto rounded-3xl border border-emerald-100 bg-white p-0 shadow-2xl [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:max-w-2xl">
                <DialogHeader className="border-b border-emerald-100 bg-gradient-to-r from-white to-emerald-50 px-6 py-5">
                    <DialogTitle className="text-2xl font-semibold text-slate-900">Edit Employee</DialogTitle>
                    <DialogDescription className="text-sm text-slate-500">
                        Update {employee.name}'s account, profile details, and image from one place.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 p-6">
                    <EmployeeAvatarField
                        ...
                        name={form.data.name}
                        helperText={employee.avatar ? 'Choose a new image to replace the current one.' : 'Upload a profile image for the employee.'}
                        onChange={(file) => form.setData('avatar', file)}
                    />
