            ...
            <div className="space-y-6 p-4 sm:p-6">
                <div className="overflow-hidden rounded-3xl border border-emerald-100 bg-gradient-to-r from-white via-emerald-50/70 to-cyan-50/70 shadow-sm">
                    <div className="px-6 py-6 lg:px-8">
                        ...
                        <p className="mt-2 text-sm text-slate-600">Issue reports and feature requests sent by employees.</p>
                    </div>
                </div>
                ...
                {feedbackMessages.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {feedbackMessages.map((message) => (
                            <div key={message.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                ...
                                <h3 className="mt-3 text-base font-semibold text-slate-900">{message.subject}</h3>
                                <p className="mt-1 text-sm text-slate-500">
                                    From: {message.employee_name ?? 'Unknown employee'} ({message.from_email})
                                </p>
                ...
                {replyingTo && (
                    <form onSubmit={handleReplySubmit} className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <p className="text-sm font-semibold text-slate-900">Reply to {replyingTo.employee_name ?? 'Employee'}</p>
                        <p className="text-xs text-slate-500">{replyingTo.subject}</p>
