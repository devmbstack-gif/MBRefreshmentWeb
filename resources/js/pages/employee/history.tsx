            ...
            <div className="space-y-6">
                <div className="overflow-hidden rounded-3xl border border-emerald-100 bg-gradient-to-r from-white via-emerald-50/70 to-cyan-50/70 shadow-sm">
                    <div className="flex flex-col gap-6 px-6 py-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-2">
                            <HeroMiniStat label="Total logs" value={usages.length} />
                            <HeroMiniStat label="Total used" value={usages.reduce((sum, usage) => sum + usage.quantity_used, 0)} />
                        </div>
                    </div>
                </div>
                ...
                {usages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-white py-20 text-center shadow-sm">
                        ...
                        <p className="text-base font-semibold text-slate-800">No usage history yet</p>
                    ...
                    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                        ...
                        <div className="divide-y divide-slate-100">
                            {usages.map((usage) => (
                                <div key={usage.id} className="flex items-center justify-between gap-4 px-6 py-4">
                                    <div className="flex min-w-0 items-center gap-4">
                                        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br ${getCatConfig(usage.item_category).bg}`}>
                                            <span className="text-xl" aria-hidden>{getCatConfig(usage.item_category).emoji}</span>
                                        </div>
                                        <div>
                                            <p className="text-sm font-semibold text-slate-900">{usage.item_name}</p>
                                            <div className="mt-0.5 flex items-center gap-2">
                                                <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold text-white ${getCatConfig(usage.item_category).pill}`}>
                                                    {getCatConfig(usage.item_category).label}
                                                </span>
                                                <span className="text-xs text-slate-500">Qty: {usage.quantity_used}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex shrink-0 items-center gap-1.5 text-xs text-slate-500">
                                        ...
                                        <span className="hidden sm:inline">{usage.used_at}</span>
                                        <span className="sm:hidden"><Clock className="h-3.5 w-3.5" /></span>
                                    </div>
                                </div>
                            ))}
                        </div>
