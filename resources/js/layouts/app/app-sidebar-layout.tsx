    ...
    return (
        <div className="min-h-screen bg-[#f6f7fb]">
            ...
            <main className="min-w-0 transition-all duration-200" style={{ marginLeft: `${sidebarWidth}px` }}>
                    ...
                    <div className="flex h-full items-center justify-between gap-4 px-6">
                        <div ref={searchWrapRef} className="relative w-full max-w-xl">
                            ...
                            <input
                                ...
                                placeholder="Search..."
                                autoComplete="off"
                                className="w-full rounded-full border border-emerald-200/80 bg-gray-50 py-2 pl-9 pr-4 text-sm shadow-sm transition-colors focus:border-primary/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary/15"
                            />
                            {searchResults.length > 0 && (
                                <div className="absolute left-0 right-0 top-full z-50 mt-2 max-h-80 overflow-auto rounded-2xl border border-gray-200 bg-white py-1 shadow-xl">
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                            ...
                            {!isAdmin && (
                                <Button
                                    ...
                                    className="relative h-9 w-9 text-gray-600 hover:text-gray-900"
                                    onClick={() =>
                                        toast.info('This notifications panel is for admin only.')
                                    }
                            ...
                            {auth.user && (
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <button
                                            type="button"
                                            className="flex items-center gap-2 rounded-full py-1 pl-1 pr-2 outline-none ring-offset-2 hover:bg-gray-100 focus-visible:ring-2 focus-visible:ring-primary/30"
                                        >
                                    ...
                                    >
                                        <DropdownMenuItem asChild className="cursor-pointer rounded-xl px-3 py-2.5 focus:bg-gray-50">
                                            <Link href={profileEdit()} className="flex items-center gap-3 text-gray-900">
                                                <User className="h-4 w-4 text-gray-400" />
                                                Profile
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem asChild className="cursor-pointer rounded-xl px-3 py-2.5 focus:bg-gray-50">
                                            <Link href={securityEdit()} className="flex items-center gap-3 text-gray-900">
                                                <Settings className="h-4 w-4 text-gray-400" />
                                                Settings
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuSeparator className="my-1 bg-gray-100" />
                                        <DropdownMenuItem asChild className="cursor-pointer rounded-xl px-3 py-2.5 text-red-600 focus:bg-red-50 focus:text-red-600">
                                            <Link href={logout()} method="post" as="button" className="flex w-full items-center gap-3">
                ...
                <div className="min-h-screen bg-[#f6f7fb] p-6 pt-20">{children}</div>
            </main>
        </div>
    );
}
